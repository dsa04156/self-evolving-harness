...
| Operations owner | `seh-operations` control container | harness/deployment registry, protocol manifest, public keys | session lease/checkpoint/event intake and artifact intake through brokers | accepts operator control and runtime events; emits start/interrupt/terminate/resume/recover/finalize/delegation | operations signing key | local authenticated control sockets only | control request rate, lease count, checkpoint bytes | session transitions including termination, leases, operation receipts |
...
| Promoter | `seh-promoter` minimal decision container | protocol, qualification/deployment policies, qualification projection, evaluation/gate/canary results, audit heads, production pointer | no filesystem registry write; authenticated CAS request only | accepts qualification/deployment requests; emits signed `PromotionDecision`, `DeploymentDecision`, and CAS request | promoter signing key | local audit and deployment-registry sockets only; no provider | decision count and short wall-clock/CPU limit; zero model calls | qualification and deployment decisions; cannot write pointer directly |
...
| Proposer | redacted evidence and allowed task content | none at any time | none | none | none |
...
| Promoter | aggregate signed mine results | internal signed gate result during the one-shot transaction | none for decisions | none for decisions | none for decisions |
