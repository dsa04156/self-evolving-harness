...
| 3. Operations Control Plane | `start`, `submit`, `observe`, `interrupt`, `terminate`, `resume`, `recover`, `validate`, `finalize`, `retire`, checkpoints and ownership | harness mutation |
...
1. **Runtime principal** reads one resolved harness manifest, executes tools through the permission
   broker, and appends events. It cannot deploy a harness version.
...
- **Component registry:** immutable component payload blobs/manifests and append-only external provenance
  records.
- **Harness registry:** immutable whole-version manifests, append-only lineage/qualification records, and
  one atomic `production` whole-harness channel pointer.
...
2. Parent and child/subagent sessions use the same harness version unless a new top-level session is
   explicitly started after an approved version is deployed.
3. Component payloads and harness manifests are immutable; qualification and channel deployment are
...
12. Abnormal termination revokes descendants/capabilities, stops jobs, seals accounting/evidence, and
    ends in `terminated`; no resume is possible.
...
[ADR-0001](adr-0001-runtime-and-evaluator-stack.md) and remains pending Gate 1 approval. The pre-contract
