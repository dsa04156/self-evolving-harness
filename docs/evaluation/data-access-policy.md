...
- HarnessFaultBench IDs and role membership are in `benchmarks/splits.json`; executable semantics are
  frozen by `FIXTURE_SPEC.md`. The 28 visible `D_mine` bodies are generated by the public deterministic
  builder. Gate/final bodies are not instantiated and cannot be requested through that builder.
...
- Gate/final task bodies live outside candidate repositories when they are independently authored; Git
  worktrees are not access control. Public mine fixtures are development data and carry no secrecy
  claim.
