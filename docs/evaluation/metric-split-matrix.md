...
| HarnessFaultBench `D_gate` single fault | top-1 ≥70%, top-3 ≥90% | one protocol-lifetime readiness evaluation after attribution implementation is frozen | none; internal selector/audit evidence until final closure | yes, as protocol admission—not candidate-specific tuning | attribution readiness |
...
| Terminal-Bench selection validation (`D_gate`, 10 public tasks) | paired pass rate, pass→fail/fail→pass, cost, violations | one non-adaptive candidate selection batch in the protocol lifetime | none; internal selector/audit evidence until final closure | yes | selection only; no confirmatory performance claim |
...
4. `D_gate` is unlocked once per protocol and results remain promoter/audit-only until final closure.
   Any later human release makes it exploratory; subsequent confirmatory work requires a fresh gate.
